...
parameters <- list(carrying_capacity = 10000, 
                  max_offspring = 1000,
                  competition_coefficient = 0.000001,
                  short_range_distance = 20000,
                  long_range_distance = 100000,
                  short_range_probability = 0.83,
                  emigration_probability = 0.0,
                  no_introduced_per_cell = 1000, 
                  Year = 1000,
                  Species = 10)
```
...
```{r, echo=TRUE, warning=FALSE, message=FALSE}
demos <- Demography(Rmax = parameters$max_offspring, bc = parameters$competition_coefficient, ReproductionType = 0)
demos
```
...
disps <-  Dispersal(Emigration = Emigration(EmigProb = parameters$emigration_probability), Transfer =  DispersalKernel(DoubleKernel = T, Distances = dist)) + Settlement(MaxSteps = 0, Settle = 2)
disps
```
...
`Mean Lag End` = mean(lagresults$Laglength,na.rm=T)
lagspecies = table(lagresults$Lag) |> as.data.frame()
lag_count = lagspecies[lagspecies$Var1 == TRUE, "Freq"]
```

output table for lag assessment

```{r, lagtable,  echo=FALSE, message=FALSE}
parameters_table = as.tibble(parameters)
lag_table = tibble(parameters_table, LagSpecies = lag_count, `Mean Lag End` = `Mean Lag End`, `% Lag Species` = 100* lag_count/sum(lagspecies$Freq), `% Lag Length of Years Assessed` = 100 * `Mean Lag End`/Year)

if(file.exists("lag_table.csv") == FALSE) {
  write.csv(lag_table, "lag_table.csv")
} else {
  lag_table = rbind(read.csv("lag_table.csv", row.names = "X"), lag_table)
  write.csv(lag_table, "lag_table.csv")
}
